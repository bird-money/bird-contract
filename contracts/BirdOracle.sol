...
contract BirdOracle is BirdOracleInterface {
    ...
    uint256 minConsensus = 2; //minimum number of consensus before confirmation
    uint256 birdNest = 3; // bird consensus count
    uint256 trackId = 0; //increament id's
    ...
    struct BirdRequest {
        ...
        string key;
        uint256 value;
        uint256 arrivedBirds;
        bool resolved;
        address addr;
        mapping(uint256 => uint256) response;
        mapping(address => uint256) nest;
    }
     ...
     */
    event OffChainRequest(uint256 id, string url, string key);

    /**
     * To call when there is consensus on final result
     */

    event UpdatedRequest(uint256 id, string url, string key, uint256 value);

    // container for the ratings
    mapping(address => uint256) ratings;

    function newChainRequest(string memory _url, string memory _key) public {
        uint256 length =
            onChainRequests.push(
                BirdRequest(trackId, _url, _key, 0, 0, false, address(0))
            );
         ...
         */
        address trustedBird1 =
            address(0x35fA8692EB10F87D17Cd27fB5488598D33B023E5);
        address trustedBird2 =
            address(0x58Fd79D34Edc6362f92c6799eE46945113A6EA91);
        address trustedBird3 =
            address(0x0e4338DFEdA53Bc35467a09Da483410664d34e88);
         ...
         */
        emit OffChainRequest(trackId, _url, _key);
    ...
    function updatedChainRequest(uint256 _id, uint256 _valueResponse) public {
        ...
        if (trackRequest.resolved) return;
        ...
        if (trackRequest.nest[msg.sender] == 1) {
             ...
             * TODO: refactor
             */
            uint256 tmpI = trackRequest.arrivedBirds;
            trackRequest.response[tmpI] = _valueResponse;
            trackRequest.arrivedBirds = tmpI + 1;

            uint256 currentConsensusCount = 1;

            /**
             * Loop through list and check if min consensus has been reached
             */

            for (uint256 i = 0; i < tmpI; i++) {
                uint256 a = trackRequest.response[i];
                uint256 b = _valueResponse;
    ...
    function getRatingByAddressString(string memory _str)
        public
        view
        returns (uint256)
    {
    ...
    function extractAddress(string memory url)
        internal
        pure
        returns (string memory)
    {
        bytes memory strBytes = bytes(url);
        uint256 index = strBytes.length - 1;
        while (index >= 0) {
            if (strBytes[index] == "/" || strBytes[index] == "\\") break;
            index--;
        }
